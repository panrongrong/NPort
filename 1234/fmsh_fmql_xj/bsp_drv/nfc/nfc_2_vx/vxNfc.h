...
#ifndef __INC_VX_NFC_H__
#define __INC_VX_NFC_H__
...
} vxT_BBT_DESC; 	 /* Bad block table descriptor */
...
typedef struct _t_nfc_ctrl_
{
	...
	vxT_NAND_INFO   NAND;              /*  Part geometry */
	
	UINT32 res[16];                    /* for data error */
	
	vxT_BBT_DESC    BBT_DESC;           /* Bad block table descriptor */
	vxT_BBT_DESC    BBT_DESC_MIRROR;    /* Mirror BBT descriptor */

	vxT_BB_PATTERN  BB_PATTERN;         /* Bad block pattern */
	
    UINT8  info_BB[NAND_MAX_BLOCKS >> 2];
    UINT32 map_BB[NAND_MAX_BADBLKS];                           /**< Nandflash Bad Block Info */
    
   /* UINT8  txBuf[NAND_MAX_PAGE_DATASIZE+NAND_MAX_PAGE_SPARESIZE];   /** Buffer to send (state) */
   /* UINT8*pRxBuf;	                                                      * Buffer to receive (state) */
...
typedef struct _t_nfc_
{
	int nfc_x;
	
	vxT_NFC_CTRL * pNfcCtrl;

	int init_flag;  /* 1-init_ok, 0-no_init/init_fail*/
...
int nfcCtrl_Get_FlashInfo(vxT_NFC* pNfc, int);
