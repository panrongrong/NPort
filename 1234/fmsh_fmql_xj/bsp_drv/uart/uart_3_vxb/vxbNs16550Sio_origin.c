...
--------------------
10sep14,d_l  read RBR after RX FIFO is enabled. (VXW6-83413)
...
#if 0
#define NS16550_DBG_MSG(level,fmt,a,b,c,d,e,f) printf(fmt,a,b,c,d,e,f)
#endif
...
/*
 *  Technobox 4960 PCI card
 */

#define UART_VENDOR_TECHNOBOX        0x10B5
#define UART_DEVID_TECHNOBOX_4960    0x1023
    { UART_DEVID_TECHNOBOX_4960, UART_VENDOR_TECHNOBOX },	
...
\i <checkLSRBeforeReadData>
A BOOL variable specifies whether the driver needs checking LSR_DR bit before
reading RBR. Some special chips, i.e. DM 37X, will produce a data exception
if we read RBR directly without checking LSR_DR bit. In this case, it is 
...
static SIO_DRV_FUNCS ns16550vxbSioDrvFuncs_2 =
{
#if 1
    (int (*)())NULL,
    (int (*)())NULL,
    ...
    (int (*)())ns16550vxbPollInput,
#endif	/* _VXBUS_BASIC_SIOPOLL */
    (int (*)())NULL  //(int (*)(SIO_CHAN *,char))ns16550vxbPollOutput
};
...
void ns16550SioRegister(void)
    ...
    /* the return value is not needed or used */

    (void) vxbDevRegister((struct vxbDevRegInfo *)&ns16550vxbDevRegistration);
    }
...
#define XLP_PCI_DEVICE_SCRATCH_REG0 0xE0 /* PCI Device scratch registers 0 */
        ...
        if (pPciDevInfo->pciVendId == UART_VENDOR_TECHNOBOX)
            {
            if (pPciDevInfo->pciDevId == UART_DEVID_TECHNOBOX_4960)
                {
                /*
                 * BARs already set correctly, so we don't need to do anything
                 * special here except to disable the second BAR.
                 */
                pDev->pRegBase[1] = 0;
                }
            }
    ...
    for ( i = 0 ; i < VXB_MAXBARS ; i++ )
        ...
        pChan->checkLSRBeforeReadData = FALSE;
        ...
        /*
         * resourceDesc {
         * The checkLSRBeforeReadData resource specifies whether the driver 
         * needs checking LSR_DR bit before reading RBR. If this property is 
         * not explicitly specified, the driver doesn't check LSR by default.}
         */

        if ( (pHcf != NULL) &&
             (devResourceGet (pHcf, "checkLSRBeforeReadData", HCF_RES_INT,
             (void *) &pChan->checkLSRBeforeReadData) != OK))
            pChan->checkLSRBeforeReadData = FALSE;
        ...
        pChan->pDev = pDev;
        pChan->regIndex = i;
        (void)vxbRegMap (pDev, i, &pChan->handle);
...
LOCAL void ns16550vxbSioChannelConnect
    ...
    (void)vxbIntConnect(pDev, interruptIndex, ns16550vxbInt, pDev);
	...
	(void)vxbIntEnable(pDev, interruptIndex, ns16550vxbInt, pDev);
    }
    ...
    )
{
    ...
    pChan->pDrvFuncs = &ns16550vxbSioDrvFuncs_2;
}
...
LOCAL void ns16550vxbInitChannel
    ...
    (void)ns16550vxbBaudSet(pChan, (UINT)pChan->baudRate);
...
LOCAL STATUS ns16550vxbOptsSet
	...
	case PARENB|PARODD:
	    pChan->lcr |= LCR_PEN;
        pChan->ier |= IER_ELSI;   /* Enable receiver line status interrupt.*/
        break;
	case PARENB:
    ...
    if (pChan->checkLSRBeforeReadData)
        {
...
LOCAL STATUS  ns16550vxbBaudSet
    ...
    /*
     * TODO: set LCD_DLAB require USR_busy must be 0
     */
#if 0  /* jc*/
...
LOCAL STATUS ns16550vxbIoctl
	...
	case SIO_BAUD_SET:
		...
		status = ns16550vxbBaudSet (pChan, (UINT)arg);
	    break;
...
* ERRNO
*/
char g_1key_pressed = 0;
char g_2key_pressed = 0;
    ...
    )
{
    ...
    while (TRUE)
	{
		...
		/* jc: for 2key function*/
#ifdef VX_BOOT_ACK_2KEY
			if (pChan->channelNo == CONSOLE_TTY)
		    {
		    	/* #define KEY_ESC   (0x1B) 	// 27	0x1B	ESC*/
				if (inchar == 0x1B)   /* ESC key*/
				{   
					flag_2key = inchar;
					g_1key_pressed = inchar;
				}
				else if (flag_2key == 0x1B)
				{
					if (inchar != 0x1B)  /* (('1'<= inchar)&&(inchar<'5')) */
					{
						g_2key_pressed = inchar;
						logMsg("key1=0x%x, key2=0x%X \n", g_1key_pressed, g_2key_pressed, 3,4,5,6);
					}
					else
					{
						(*pChan->putRcvChar) (pChan->putRcvArg, flag_2key);
...
LOCAL int ns16550vxbPollInput
    ...
    if (pollStatus & LSR_PE)
		{
		char chr;
		REG_GET(RBR, pChan, chr);
		
		NS16550_DBG_MSG(500,"ns16550vxbPollInput(): LSR_PE\n",
				1,2,3,4,5,6);
		return(EAGAIN);
		}
...
void ns16550vxbSioShow
	...
	while (pMethod->devMethodId != 0 && pMethod->handler != 0)
	    {
	    printf("\t    0x%x => '%s'\n",
...
int test_com1_snd(void)
{
	...
	/**/
	/* it's important to init uart before tx data*/
	/**/
#ifdef DRV_UART_1_OPEN  /* uart_0: console default, uart_1: com port*/
	extern int vxInit_Uart(int ctrl_x);
	vxInit_Uart(ttynum);
#endif


	sprintf(fd_str, "/tyCo/%d", ttynum);	
	...
	ioctl(fd, FIOSETOPTIONS, OPT_RAW);         /* ËÆæÁΩÆ‰∏≤Âè£ÁöÑÊï∞ÊçÆÊ®°Âºè */
	ioctl(fd, FIOBAUDRATE, rate);              /* ËÆæÁΩÆ‰∏≤Âè£ ÁöÑÊ≥¢ÁâπÁéá       */
	ioctl(fd, FIOFLUSH, 0);                    /* Ê∏ÖÁ©∫‰∏≤Âè£ÁöÑÁºìÂÜ≤Âå∫          */
    /*ioctl(fd, FIOSETOPTIONS, OPT_TERMINAL);*/

#if 0
	/* Âà§Êñ≠com2Êé•Êî∂Êï∞ÊçÆÁºìÂÜ≤Âå∫ÊòØÂê¶ÊúâÊï∞ÊçÆÂà∞Êù• */
	...
	sprintf(buffer, "abcdefghijklmn_%d \n", g_test_com1++);
	printf("The src str: %s  \n", buffer);

	bytes_out = write(fd, (char*)(&buffer[0]), num);   /* ÂæÄ‰∏≤Âè£ÂÜôÂÖ•‰∏Ä‰∏™Â≠óÁ¨¶‰∏≤*/
	
	taskDelay(10);
	printf("The str snd: %d bytes! \n", bytes_out);
...
int task_com1_rcv(int fd)
{
	...
	while (1)
	{
		...
		ioctl(fd, FIONREAD, (int) &readCnt1); /* Âà§Êñ≠com2Êé•Êî∂Êï∞ÊçÆÁºìÂÜ≤Âå∫ÊòØÂê¶ÊúâÊï∞ÊçÆÂà∞Êù• */
...
int test_com1_rcv(void)
{
	int com2_Fd;
	
	com2_Fd = open("/tyCo/1", 2, 0);     /* ÊâìÂºÄ‰∏≤Âè£1ÔºåÂç≥serial2<->com2 */

	/* 
	ËÆæÁΩÆ‰∏≤Âè£1Ôºå‰∫¶Âç≥com2ÁöÑÊ≥¢ÁâπÁéá115200Ôºå8Êï∞ÊçÆ‰∏∫Ôºå1ÂÅúÊ≠¢‰ΩçÔºåÊó†Ê†°È™å‰Ωç
	*/
	...
	/* ÂèëËµ∑Êé•ÂèóÊï∞ÊçÆÁöÑ‰ªªÂä° */
	taskSpawn("com1Rcv", 160, 0, 0x2000, (FUNCPTR)task_com1_rcv, com2_Fd, 0,0,0,0,0,0,0,0,0);
...
//STATUS serialInit(UART_CHAN* uart, UINT32 udBaseAddr, char* DevName, unsigned int intBit)
STATUS serial_Init(NS16550VXB_CHAN* uart)
{
    int i=0;
    int oldlevel=0;
    char bResult = 0;
	...
	uart->regDelta = 4;
	uart->regs = 0x30000000; //UART_1_BASE_ADR;  // udBaseAddr;
	uart->baudRate = 115200;  //CONSOLE_BAUD_RATE;
	uart->xtal = UART_XTAL_FREQ;
	
	//spinLockIsrInit (&(uart->spinLock), 0);

	uart->level = 57;  //RS485_IRQ_NUM;	
	
	// st16552DevInit(uart);
	ns16550vxbDevInit_2(uart);
		
#if 0
	intDisable(uart->level);	

    oldlevel = intLock(&uart->spinLock);//’‚∏ˆµÿ∑Ωµƒ”√∑®≤ª÷™µ¿ «∑Ò”–Œ Ã‚
	...
	MYUART_REG_WRITE(uart, IER, RxFIFO_BIT/*|IER_ETHREI*/);			//¥Úø™Ω” ’÷–∂œ
	...
	regV = regV |intBit;
	MY_REG_WRITE(uart->fpgaIntBase+FPGA_INT_INT_EN, regV);/*ø™÷–∂œ*/

	MY_REG_READ(uart->fpgaIntBase+FPGA_INT_INT_MODE, regV);
	regV = regV & (~intBit);
	MY_REG_WRITE(uart->fpgaIntBase+FPGA_INT_INT_MODE, regV);/*÷–∂œƒ£ Ω--µÁ∆Ω÷–∂œ*/
	#endif
	
	/* ¥Úø™¬ﬂº≠ªπªÿπ¶ƒ‹ */
//	MYUART_REG_WRITE(uart, FPGA_LOOP, 0x1); 

//	plIntModeSet(uart->level,INTR_TRIGGER_HIGH_LEVEL);/*∏ﬂµÁ∆Ω¥•∑¢*/
	/* ø™÷–∂œ */
...
STATUS test_uartDev_Init(void)
{
	//uartDev[0].RS485 = 1;
/*	NS16550VXB_CHAN uartDev;
	
	serial_Init(&uartDev);
		
	(void)ttyDevCreate ("/Rs485/0", &uartDev, UART_BUFFER_SIZE, UART_BUFFER_SIZE);
*/	
	(void) ttyDevCreate ("/Rs485/0", sysSerialChanGet(1), 512, 512);
	(void) ttyDevCreate ("/Rs485/1", sysSerialChanGet(1), 512, 512);
	(void) ttyDevCreate ("/Rs485/2", sysSerialChanGet(1), 512, 512);
	//(void) ttyDevCreate ("/Rs485/1", sysSerialChanGet(3), 512, 512);
	return OK;
}

#endif  /* #ifdef DRV_UART_1_OPEN */
