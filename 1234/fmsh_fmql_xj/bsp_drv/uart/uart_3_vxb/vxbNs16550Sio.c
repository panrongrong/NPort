...
/* local defines       */
/* zjb*/
#define VXB_NS16550_CONSOLE_BAUD_RATE	 (CONSOLE_BAUD_RATE) /* 9600 */
...
#define NS16550_MAX_RATE 1152000/*115200*/
    ...
    )
{
    ...
    tmp = 16 * baud;	
    tmp = (float)(pChan->xtal / tmp); 
	
    divisor = (UINT32)tmp; 
    divisor_f = tmp - divisor;  
    divisor_f = divisor_f * 16;  
	
	if (((UINT32)(divisor_f * 10) % 10) < 5)  /* 鍥涜垗浜斿叆*/
	{
	...
	while (timeOut < 0)
	{
		REG_GET(USR,pChan,usrReg);
		
		if ((usrReg & USR_BUSY) == USR_BUSY)
		{
			uartSetdelay();
		}
		else
	...
	REG_GET(LCR,pChan,lcrReg);
    value = (LCR_DLAB | lcrReg);
    REG_SET(LCR, pChan, value);

    /* Set divisor latches. */
    REG_SET(DLL, pChan, dll);
	REG_SET(DLM, pChan, dlh);
    REG_SET(DLF, pChan, dlf);
	
    /* Restore line control register */
    REG_SET(LCR, pChan, lcrReg);	
...
int test_com1_snd(void)
{
	...
	*/
	...
	ioctl(fd, FIOSETOPTIONS, OPT_RAW);         /* 璁剧疆涓插彛鐨勬暟鎹ā寮�*/
	ioctl(fd, FIOBAUDRATE, rate);              /* 璁剧疆涓插彛 鐨勬尝鐗圭巼       */
	ioctl(fd, FIOFLUSH, 0);                    /* 娓呯┖涓插彛鐨勭紦鍐插尯          */
    /*ioctl(fd, FIOSETOPTIONS, OPT_TERMINAL);*/

#if 0
	/* 鍒ゆ柇com2鎺ユ敹鏁版嵁缂撳啿鍖烘槸鍚︽湁鏁版嵁鍒版潵 */
	...
	bytes_out = write(fd, (char*)(&buffer[0]), num);   /* 寰�覆鍙ｅ啓鍏ヤ竴涓瓧绗︿覆*/
...
int task_com1_rcv(int fd)
{
	...
	while (1)
	{
		...
		ioctl(fd, FIONREAD, (int) &readCnt1); /* 鍒ゆ柇com2鎺ユ敹鏁版嵁缂撳啿鍖烘槸鍚︽湁鏁版嵁鍒版潵 */
...
int test_com1_rcv(void)
{
	...
	com2_Fd = open("/tyCo/1", 2, 0);     /* 鎵撳紑涓插彛1锛屽嵆serial2<->com2 */

	/* 
	璁剧疆涓插彛1锛屼害鍗砪om2鐨勬尝鐗圭巼115200锛�鏁版嵁涓猴紝1鍋滄浣嶏紝鏃犳牎楠屼綅
	*/
