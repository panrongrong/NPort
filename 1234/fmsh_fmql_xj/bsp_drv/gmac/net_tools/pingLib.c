...
01u,07nov02,ant  New local pingLen initialized to global _pingTxLen and used throughout
		 the code instead of _pingTxLen.
01t,07oct02,wie  print stack number for virtual stack debug support
01s,07may02,ant  New global int	_pingSockBufSize defined (setsockopt(SO_SNDBUF) and
...
01o,23feb01,rae  merged from 01n of tor2_0.barney (base: 01k):
		 Added PING_OPT_NOHOST flag to deal with SPR 22766
                 corrected ping error for pingRxPrint() (SPR22571)
...
int	_pingSockBufSize = RAWSNDQ;		/* size of socket buffer */
...
LOCAL void pingFinish (int taskId);

STATUS pingLibInit (void);


#ifndef _WRS_KERNEL
LOCAL u_short checksum (u_short * pAddr, int len);
#endif
...
* timeout, _pingTxTmo) to return, check your resolver settings to see if
...
STATUS ping
	...
	_pingTxLen = pingLen = PING_MAXPACKET - 20;
	}
    else if (pingLen < 8)
	{
	if (!(options & PING_OPT_SILENT))
    ...
    tickTmo = _pingTxTmo * sysClkRateGet();
     ...
     */
    if (pingHookCount == 0 &&
	taskDeleteHookAdd ((FUNCPTR) pingFinish) == ERROR)
	{
	free ((char *) pPS);
    ...
    /* check _pingSockBufSize, (it should be bigger than _pingTxLen + 35 ?) */

    if ((pPS->dataLen + 44) > _pingSockBufSize)
        {
	if (!(options & PING_OPT_SILENT))
    ...
    /* set the send and receive buffer of the socket if the value in the global
       _pingSockBufSize is different than default one */

    if (_pingSockBufSize != RAWSNDQ) 
	{
	if (setsockopt(pPS->pingFd, SOL_SOCKET, SO_SNDBUF,
		       (char *)&_pingSockBufSize,
		       sizeof(_pingSockBufSize)) < 0) 
	    {
	 ...
	 * messages. However, this seems to be the only place to put it.
         ...
         */
        #ifdef _WRS_KERNEL
       /* _icmpstat.icps_outhist [ICMP_ECHO]++;  // jc for test*/
        #endif
...
check_fd_again:					/* Wait for ICMP reply */
	...
	now = tickGet();
	if (pingRxPrint (pPS, ix, &from, now, FALSE) == ERROR)
	    goto check_fd_again;
...
release:
    ...
    pingRxPrint (pPS, ix, &from, now, TRUE);
...
LOCAL STATUS pingRxPrint
	...
	if (*fromHostName != 0x00)
	    ...
	    {
	    	#if 1
	            triptime = (int)((now - *((ulong_t *) icp->icmp_data)) *
		        (1000 / pPS->clkTick));	
	            printf ("time=%d. ms\n", triptime/33);
			#else
				triptime = now * 1000 / pPS->clkTick;
	            printf ("time=%d. ms\n", triptime);
			#endif
...
LOCAL void pingFinish
    ...
    for (pPS = pingHead; pPS != NULL; pPS = pPS->statNext)
	{
	if (pPS->idRx == taskId)
	    goto found;
        ppPrev = &pPS->statNext;
