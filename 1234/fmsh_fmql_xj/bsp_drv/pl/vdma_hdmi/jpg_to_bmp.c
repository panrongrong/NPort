...
BOOL LoadJpegFile(char *JpegFileName)
{
	...
	memset((char *)&bfile, 0, sizeof(BITMAPFILEHEADER));
	memset((char *)&binfo, 0, sizeof(BITMAPINFOHEADER));
 
	binfo.biSize = (DWORD)sizeof(BITMAPINFOHEADER);
	binfo.biWidth = (LONG)(ImgWidth);
	binfo.biHeight = (LONG)(ImgHeight);
	binfo.biPlanes = 1;
	binfo.biBitCount = 24;
	binfo.biClrUsed = 0;
	...
	NumColors = 0;
	printf("binfo.biWidth is %ld/n", binfo.biWidth);
	printf("binfo.biBitCount is %ld/n", binfo.biBitCount);
	LineBytes = (DWORD)WIDTHBYTES(binfo.biWidth*binfo.biBitCount);
	printf("LineBytes is %ld/n", LineBytes);
	ImgSize = (DWORD)LineBytes*binfo.biHeight;/*???????*/
	...
	bfile.bfSize = sizeof(BITMAPFILEHEADER)+sizeof(BITMAPINFOHEADER)+NumColors*sizeof(RGBQUAD)+ImgSize;
	bfile.bfOffBits = 54;/*(DWORD)(NumColors*izeof(RGBQUAD)+sizeof(BITMAPFILEHEADER)+sizeof(BITMAPINFOHEADER));*/
	BufSize = bfile.bfSize - sizeof(BITMAPFILEHEADER);
	...
	memcpy(lpImgData, (char *)&binfo, sizeof(BITMAPINFOHEADER));
	...
	if (funcret == FUNC_OK)
	{
		...
		fwrite((LPSTR)&bfile, sizeof(BITMAPFILEHEADER), 1, hfbmp);
...
void jpg_to_bmp_transfer(UINT8 * pSrc8, UINT8 * pDst8)
{
	DWORD          ImgSize;
	DWORD              BufSize, JpegBufSize;
	FILE*              hfbmp;
	...
	NumColors = 0;
	printf("binfo.biWidth is %ld \n", binfo.biWidth);
	printf("binfo.biBitCount is %ld \n", binfo.biBitCount);
	...
	if (funcret == FUNC_OK)
	{
		...
		/**/
	#if 0
		memcpy(pDst8, (LPSTR)&bfile, sizeof(BITMAPFILEHEADER));		
		pDst8 += sizeof(BITMAPFILEHEADER);
	#endif
		/**/
		/* bmp file data area*/
		/**/
		memcpy(pDst8, (LPSTR)lpImgData, BufSize);
...
/* with bmp file header info*/
void jpg_to_bmp_transfer2(UINT8 * pSrc8, UINT8 * pDst8)
{
	DWORD          ImgSize;
	DWORD              BufSize, JpegBufSize;
	FILE*              hfbmp;
