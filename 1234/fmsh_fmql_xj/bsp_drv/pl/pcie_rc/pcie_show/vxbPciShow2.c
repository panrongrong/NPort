...
#include "vxbPciLib2.h"
#include "pciDefines2.h"
#include "pciConfigLib2.h"
...
STATUS VXB_PCI_CFG_READ_2 (VXB_DEV_ID pDev,  PCI_HARDWARE * pciDev, UINT32 offset, UINT32 width, void * data)
{
...
STATUS VXB_PCI_CFG_WRITE_2 (VXB_DEV_ID pDev,  PCI_HARDWARE * pciDev, UINT32 offset, UINT32 width, UINT32 data)
{
	UINT8 PCIeBusNum, PCIeDevNum, PCIeFunNum;
	UINT32 tmp32 = 0;
	UINT32 offset_2 = 0;
...
void vxbDevGetNameUnit2
    (
    VXB_DEV_ID pDev,           /* identifier of device of interest */
    char *     buf,            /* pointer to memory location to use */
    size_t     len             /* length in bytes of buf */
    )
{
    if (buf != NULL)                              /* req: VX7-13800 */
    {
       /* (void) snprintf (buf, len, "%s:%d", pDev->pName, pDev->vxbUnit);*/
		(void) snprintf (buf, len, "%s:%d", "pcie_rc", 0);
    }
...
/*********************************************************************
*
* latL0s_2 - return string for latency levels
*
...
LOCAL const char *latL0s_2
    (
    int latVal
    )
    {
      static const char *lat_0[] =
          {
          "<64ns",
          "<128ns",
          "<256ns",
...
/*********************************************************************
*
* latL1_2 - return string for latency levels
*
* RETURNS: string for latency levels
*
* ERRNO
...
LOCAL const char *latL1_2
    (
    int latVal
    )
    {
    static const char *lat_1[] =
        {
        "<1us",
        "<2us",
        "<4us",
...
/*********************************************************************
*
* limScale_2 - returns scaling value from negative power
*
...
/*********************************************************************
*
* aspm_2 - returns string for latency level
*
...
/*********************************************************************
*
* aspmEnable_2 - returns string for which level is enabled
*
...
LOCAL const char *aspmEnable_2
    (
    int val
    )
    {
    static const char *enab_2[] =
        {
        "Disabled",
        "L0s",
        "L1",
...
/*********************************************************************
*
* indicator_2 - returns string for incator state
*
* RETURNS:  string for indicator_2 state
*
* ERRNO
*/

LOCAL const char *indicator_2
    (
    int val
    )
{
    static const char *state_2[] =
        {
        "Unknown",
        "On",
        "Blink",
...
/*******************************************************************************
*
* vxbPciConfigInByte_2 - read one byte from the PCI configuration space
*
...
LOCAL STATUS vxbPciConfigInByte_2
    (
    ...
    return VXB_PCI_CFG_READ_2(busCtrlID, &phard, offset, 1, pData);
    }

/*******************************************************************************
*
* vxbPciConfigInWord_2 - read one word from the PCI configuration space
*
...
LOCAL STATUS vxbPciConfigInWord_2
    (
    VXB_DEV_ID busCtrlID,
    UINT8      busNo,    /* bus number */
    UINT8      deviceNo, /* device number */
    ...
    return VXB_PCI_CFG_READ_2(busCtrlID, &phard, offset, 2, pData);
    }

/*******************************************************************************
*
* vxbPciConfigInLong_2 - read one longword from the PCI configuration space
*
...
LOCAL STATUS vxbPciConfigInLong_2
    (
    VXB_DEV_ID  busCtrlID,
    UINT8      busNo,    /* bus number */
    UINT8      deviceNo, /* device number */
    ...
    return VXB_PCI_CFG_READ_2(busCtrlID, &phard, offset, 4, pData);
    }
#endif

/*******************************************************************************
*
* vxbPciDheaderPrint2 - print a PCI device header
*
...
LOCAL void vxbPciDheaderPrint2
    (
...
/*******************************************************************************
*
* vxbPciBheaderPrint2 - print a PCI-to-PCI bridge header
*
...
LOCAL void vxbPciBheaderPrint2
    (
...
/*******************************************************************************
*
* vxbPciCBheaderPrint2 - print a PCI-to-CardBus bridge header
*
...
LOCAL void vxbPciCBheaderPrint2
    (
...
/*********************************************************************
*
* vxbPciFuncBarShow2 - show the values contained in all BARs
*
...
LOCAL void vxbPciFuncBarShow2
	...
	(void) vxbPciConfigInLong_2(busCtrlID,bus, device, function,
			offset, &barVal);
...
/*********************************************************************
*
* vxbPciStatusWordShow2 - show the decoded value of the status word
*
...
LOCAL STATUS vxbPciStatusWordShow2
    (
    VXB_DEV_ID busCtrlID,
    UINT8      bus,        /* bus */
    UINT8      device,     /* device */
   ...
   /* if(busCtrlID == NULL)*/
    ...
    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, PCI_CFG_STATUS, &status);
...
/*********************************************************************
*
* vxbPciCmdWordShow2 - show the decoded value of the command word
*
...
LOCAL STATUS vxbPciCmdWordShow2
    (
    VXB_DEV_ID busCtrlID,
    UINT8      bus,        /* bus */
    UINT8      device,     /* device */
    ...
    (void) vxbPciConfigInWord_2 (busCtrlID, bus,device,function,
...
/*********************************************************************
*
* vxbPcixCapabShow2 - show the PCI-X capabilities
*
...
LOCAL void vxbPcixCapabShow2
   ...
   /*     return;*/
    ...
    (void) vxbPciConfigInByte_2 (busCtrlID, bus, device, function, PCI_CFG_HEADER_TYPE,
                     &headerType);
    ...
    switch (headerType)
        ...
        case PCI_HEADER_TYPE0: /* Regular Device */
            {
            (void) printf("PCI-X \n");
            (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_X_CMD, &cmd);
            (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, offset + PCI_X_STATUS, &stat);
        ...
        case PCI_HEADER_TYPE_BRIDGE:
            ...
            (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_X_SECSTAT, &secStat);
            (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_X_PRISTAT, &status);
            (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, offset + PCI_X_UP_SXACTION_CTRL, &upstr);
            (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, offset + PCI_X_DWN_SXACTION_CTRL, &dwnstr);
...
/*********************************************************************
*
* vxbPcieMsixCapabShow2 - show the MSI-X capabilities
*
...
LOCAL void vxbPcieMsixCapabShow2
    (
    VXB_DEV_ID busCtrlID,
    UINT8      bus,        /* bus */
    UINT8      device,     /* device */
...
/*********************************************************************
*
* vxbPcieMsixCapabShow2 - show the MSI capabilities
*
...
LOCAL void vxbPcieMsiCapabShow2
    (
    VXB_DEV_ID busCtrlID,
    UINT8      bus,        /* bus */
    UINT8      device,     /* device */
    ...
    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_MSI_CTL, &ctlReg);
    ...
    (void) printf("Message Signaled Interrupts: 0x%x control 0x%x %s, %s, MME: %d MMC: %d\n", offset, ctlReg,
        ...
        if (ctlReg & PCI_MSI_CTL_64BIT)
            ...
            (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, offset + PCI_MSI_ADDR_HI, &addr);
            (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_MSI_DATA_64, &data);
            (void) printf("%08x", addr);
            }
        else
            {
            maskOffset = PCI_MSI_MASK_32;
            (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_MSI_DATA_32, &data);
            }
        (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, offset + PCI_MSI_ADDR_LO, &addr);
        ...
        if (ctlReg & PCI_MSI_CTL_MASK)
            {
            (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, offset + maskOffset, &maskBit);
            (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, offset + maskOffset + 4, &pendBit);
...
/*********************************************************************
*
* vxbPcieLinkCap2 - show the PCIe Link capabilities
*
...
LOCAL void vxbPcieLinkCap2
    ...
    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, offset + PCI_EXP_LNKCAP_REG, &lnkCap);

    (void) printf("\tLink: MAX Speed - %s, MAX Width - by %d Port - %d ASPM - %s\n",
           linkSpeed2(lnkCap & PCI_EXP_LNKCAP_LNK_SPD),
           (lnkCap & PCI_EXP_LNKCAP_LNK_WDTH) >> 4,
           lnkCap >> 24,
           aspm_2((lnkCap & PCI_EXP_LNKCAP_ASPM)>>10) );

    (void) printf("\t\tLatency: L0s - %s, L1 - %s\n",
           latL0s_2((lnkCap & PCI_EXP_LNKCAP_L0S_LAT) >> 12),
           latL1_2((lnkCap & PCI_EXP_LNKCAP_L1_LAT) >> 15));

    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_EXP_LNKCTL_REG, &lnkVal);

    (void) printf("\t\tASPM - %s,", aspmEnable_2(lnkVal & PCI_EXP_LNKCTL_ASPM));
    ...
    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_EXP_LNKSTA_REG, &lnkVal);
    (void) printf("\t\tSpeed - %s, Width - by %d\n",
           linkSpeed2(lnkVal & PCI_EXP_LNKSTA_LNK_SPD),
...
/*********************************************************************
*
* vxbPcieSlotCap2 - displays the PCI express slot capabilities
*
* RETURNS:  N/A
*
* ERRNO
...
LOCAL void vxbPcieSlotCap2
    ...
    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, offset + PCI_EXP_SLTCAP_REG, &sltCap);
    ...
    (void) printf("\t\tSlot #%d, MAX Slot Power Limit (Watts) %d e%d\n",
           ...
           limScale_2((sltCap & PCI_EXP_SLTCAP_PWR_SCL_LIMIT) >> 15));

    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_EXP_SLTCTL_REG, &sltCtl);
    ...
    if (sltCtl & PCI_EXP_SLTCTL_ATTN_INDCTL)
        (void) printf("\t\tAttn Indicator %s",
               indicator_2((sltCtl & PCI_EXP_SLTCTL_ATTN_INDCTL) >> 6));
    if (sltCtl & PCI_EXP_SLTCTL_PWR_INDCTL)
        (void) printf(" Power Indicator %s",
               indicator_2((sltCtl & PCI_EXP_SLTCTL_PWR_INDCTL) >> 8));
    if (sltCtl & PCI_EXP_SLTCTL_PWR_CTLRCTL)
        (void) printf(" Power Controller %s",
               (sltCtl & PCI_EXP_SLTCTL_PWR_CTLRCTL) >> 10 == 0 ?
               "on":"off");

    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_EXP_SLTSTA_REG, &sltSts);
...
/*********************************************************************
*
* vxbPcieExtCapAerShow2 - show the PCIe EXT AER capabilities
*
...
LOCAL void vxbPcieExtCapAerShow2
    (
    VXB_DEV_ID busCtrlID,
    UINT8      bus,        /* bus */
    UINT8      device,     /* device */
    ...
    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_CONTROL, &aerControl);

    (void) printf ("AER Control: 0x%x \n", aerControl);

    (void) printf ("   Uncorrectable : ");

    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_UNCRERR_MASK, &uncErrMask);

    (void) printf ("Mask 0x%X. ", uncErrMask);

    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_UNCRERR_SEVERITY, &uncErrSeverity);

    (void) printf ("Severity 0x%X \n", uncErrSeverity);

    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_UNCRERR_STATUS, &uncErrStatus);
    ...
    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_CRERR_MASK, &cErrMask);

    (void) printf ("Mask 0x%X. \n", cErrMask);

    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_CRERR_STATUS, &cErrStatus);
    ...
    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_HEADER_LOG, (UINT32 *)&aerLog[0]);
    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_HEADER_LOG + 4, (UINT32 *)&aerLog[4]);
    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_HEADER_LOG + 8, (UINT32 *)&aerLog[8]);
    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_HEADER_LOG + 12, (UINT32 *)&aerLog[12]);
    ...
    if ((type == PCI_EXP_TYPE_RC_INTR_EP) || (type == PCI_EXP_TYPE_RC_EVEN_COLE))
        {
        (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_ROOT_ERR_COMMAND, &rootErrCmd);
        ...
        (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_ROOT_ERR_STATUS, &rootErrSts);
    ...
    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_ROOT_CESIR, &tempValue);

    (void) printf ("   Error Source Identification: 0x%x 0x%x \n", tempValue & 0xffff, (tempValue >> 15) & 0xffff);

    if (aerControl & (1<<11))
        {
        (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_TLP_LOG, (UINT32 *)&aerLog[0]);
        (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_TLP_LOG + 4, (UINT32 *)&aerLog[4]);
        (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_TLP_LOG + 8, (UINT32 *)&aerLog[8]);
        (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + PCIE_AER_TLP_LOG + 12, (UINT32 *)&aerLog[12]);
...
/*********************************************************************
*
* vxbPcieExtCapShow2 - show the PCIe EXT capabilities
*
...
LOCAL void vxbPcieExtCapShow2
    ...
    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, pos, &capId);
    ...
    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, pos + 2 , &nextCap);
    version = nextCap & 0xf;
    nextCap = nextCap >> 4;
    ...
    while (capId)
        ...
        switch(capId)
            ...
            case  PCIE_EXT_CAP_ID_ERR:
                ...
                vxbPcieExtCapAerShow2 (busCtrlID, bus, device, function, pos, type);
                break;
            ...
            case  PCIE_EXT_CAP_ID_DSN:
                ...
                (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + 4 ,
                    (UINT32 *)&serialNum[0]);
                (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, pos + 8 ,
                    (UINT32 *)&serialNum[4]);
            ...
            case  PCIE_EXT_CAP_ID_LTR:
                ...
                (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, pos + 4 , (UINT16 *)&valueTemp);

                latencyTime = (valueTemp & 0x2ff) * latencyScale[(valueTemp >> 10) & 0x7];

                (void) printf("        Max Snoop Latency: %lld ns, ", latencyTime);

                (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, pos + 6 , (UINT16 *)&valueTemp);
                latencyTime = (valueTemp & 0x2ff) * latencyScale[(valueTemp >> 10) & 0x7];
        ...
        (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, nextCap , &capId);

        if (nextCap != 0)
           pos = nextCap;

        (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, nextCap + 2, &nextCap);
...
/*********************************************************************
*
* vxbPcieDevCap2 - show the PCIe Device capabilities
*
...
LOCAL void vxbPcieDevCap2
    ...
    (void) vxbPciConfigInLong_2 (busCtrlID, bus, device, function, offset + PCI_EXP_DEVCAP_REG, &devCap);
    ...
    (void) printf("\t\tAcceptable Latency: L0 - %s, L1 - %s\n",
           latL0s_2((devCap & PCI_EXP_DEVCAP_L0S) >> 6),
           latL1_2((devCap & PCI_EXP_DEVCAP_L1) >> 9));
    ...
    if (type == PCI_EXP_TYPE_UPSTREAM)
        (void) printf("\tSlot Power Limit (Watts) %d e%d\n",
               ((devCap & PCI_EXP_DEVCAP_PWR_VAL_LIMIT) >> 18),
                limScale_2((devCap & PCI_EXP_DEVCAP_PWR_SCL_LIMIT) >> 26));

    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_EXP_DEVCTL_REG, &devCtl);
...
/*********************************************************************
*
* vxbPcieCapabShow2 - show the MSI capabilities
*
* This routine displays the MSI specific information.
*
* RETURNS: N/A
...
LOCAL void vxbPcieCapabShow2
   ...
   /*     return;*/
    ...
    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_EXP_CAP_REG, &capReg);
    ...
    type = (UINT16)((capReg & PCI_EXP_CAP_PORT_TYPE) >> 4);
        ...
        vxbPcieDevCap2(busCtrlID, bus, device, function, offset, type);

        vxbPcieLinkCap2(busCtrlID, bus, device, function, offset, type);

        if (slot)
          vxbPcieSlotCap2(busCtrlID, bus, device, function, offset);

    if (type == PCI_EXP_TYPE_ROOT_PORT)
        {
        (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, offset + PCI_EXP_RTCTL_REG, &capReg);
    ...
    vxbPcieExtCapShow2 (busCtrlID, bus, device, function, type);
    }

/*********************************************************************
*
* vxbPciCapabShow2 - show the PCI capabilities
*
...
LOCAL void vxbPciCapabShow2
   ...
   /*     return;*/
    ...
    (void) vxbPciConfigInByte_2 (busCtrlID, bus, device, function, capOffset, &devCapID);
    ...
    while (devCapID)
        ...
        (void) vxbPciConfigInByte_2 (busCtrlID, bus, device, function, devCapID, &cap_id);
        (void) vxbPciConfigInByte_2 (busCtrlID, bus, device, function, devCapID+1, &cap_id_next);
        ...
        switch (cap_id)
            ...
            case PCI_EXT_CAP_MSI:
                vxbPcieMsiCapabShow2(busCtrlID, bus, device, function, devCapID);
                break;
            case PCI_EXT_CAP_HOT_SWAP:
                (void) printf("cPCI Hot Swap \n");
                break;
            case PCI_EXT_CAP_PCIX:
                vxbPcixCapabShow2(busCtrlID, bus, device, function, devCapID);
                break;
            ...
            case PCI_EXT_CAP_EXP:
                vxbPcieCapabShow2 (busCtrlID,bus, device, function, devCapID);
                break;
            case PCI_EXT_CAP_MSIX:
                vxbPcieMsixCapabShow2 (busCtrlID, bus, device, function, devCapID);
                break;
            default:
...
/*******************************************************************************
*
* vxbPciShowInit2 - initialize the show routines
*
...
/*******************************************************************************
*
* vxbPciCtrlShow2 - list all registered PCI/PCIe controller device ID
*
...
void vxbPciCtrlShow2 (void)
    ...
    while (pNode != NULL)
        ...
        vxbDevGetNameUnit2(pPciCtrl->pDev, buf, 128);
...
/*******************************************************************************
*
* vxbPciDeviceShow2 - print PCI device information under PCI bus
*
...
STATUS vxbPciDeviceShow2
   ...
   /*     return(ERROR);*/
	...
	for (deviceNo=0; deviceNo < 1; deviceNo++)  /* only for pcie_rc in PL*/
        {
        for (func = 0; func < 8; func++)
            ...
            (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, func, PCI_CFG_VENDOR_ID,
                             (UINT16 *)&vendorId);
            ...
            deviceId = 0xffff;
            (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, func, PCI_CFG_DEVICE_ID,
                             (UINT16 *)&deviceId);

            (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, func, PCI_CFG_REVISION,
                             &classCode);

            (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, func, PCI_CFG_HEADER_TYPE,
                             &headerType);
...
/*******************************************************************************
*
* vxbPciHeaderShow2 - print a header of the specified PCI device
*
...
STATUS vxbPciHeaderShow2
   ...
   /*     return(ERROR);*/
    ...
    (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_HEADER_TYPE,
                     (UINT8 *)&pD->headerType);

    if (pD->headerType & 0x01)      /* PCI-to-PCI bridge */
        {
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_VENDOR_ID,
                         (UINT16 *)&pB->vendorId);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_DEVICE_ID,
                         (UINT16 *)&pB->deviceId);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_COMMAND,
                         (UINT16 *)&pB->command);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_STATUS,
                         (UINT16 *)&pB->status);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_REVISION,
                         (UINT8 *)&pB->revisionId);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_PROGRAMMING_IF,
                         (UINT8 *)&pB->progIf);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_SUBCLASS,
                         (UINT8 *)&pB->subClass);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CLASS,
                         (UINT8 *)&pB->classCode);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CACHE_LINE_SIZE,
                         (UINT8 *)&pB->cacheLine);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_LATENCY_TIMER,
                         (UINT8 *)&pB->latency);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_HEADER_TYPE,
                         (UINT8 *)&pB->headerType);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BIST,
                         (UINT8 *)&pB->bist);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BASE_ADDRESS_0,
                         (UINT32 *)&pB->base0);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BASE_ADDRESS_1,
                         (UINT32 *)&pB->base1);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_PRIMARY_BUS,
                         (UINT8 *)&pB->priBus);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_SECONDARY_BUS,
                         (UINT8 *)&pB->secBus);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_SUBORDINATE_BUS,
                         (UINT8 *)&pB->subBus);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_SEC_LATENCY,
                         (UINT8 *)&pB->secLatency);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_IO_BASE,
                         (UINT8 *)&pB->ioBase);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_IO_LIMIT,
                         (UINT8 *)&pB->ioLimit);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_SEC_STATUS,
                         (UINT16 *)&pB->secStatus);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_MEM_BASE,
                         (UINT16 *)&pB->memBase);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_MEM_LIMIT,
                         (UINT16 *)&pB->memLimit);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_PRE_MEM_BASE,
                         (UINT16 *)&pB->preBase);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_PRE_MEM_LIMIT,
                         (UINT16 *)&pB->preLimit);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_PRE_MEM_BASE_U,
                         (UINT32 *)&pB->preBaseUpper);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_PRE_MEM_LIMIT_U,
                         (UINT32 *)&pB->preLimitUpper);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_IO_BASE_U,
                         (UINT16 *)&pB->ioBaseUpper);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_IO_LIMIT_U,
                         (UINT16 *)&pB->ioLimitUpper);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_ROM_BASE,
                         (UINT32 *)&pB->romBase);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BRG_INT_LINE,
                         (UINT8 *)&pB->intLine);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BRG_INT_PIN,
                         (UINT8 *)&pB->intPin);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BRIDGE_CONTROL,
                         (UINT16 *)&pB->control);
        vxbPciBheaderPrint2 (pB);

        if (pB->status & PCI_STATUS_NEW_CAP)
            vxbPciCapabShow2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CAP_PTR);
        ...
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BASE_ADDRESS_0,
                         (UINT32 *)&pCB->base0);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_CAP_PTR,
                         (UINT8 *)&pCB->capPtr);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_SEC_STATUS,
                         (UINT16 *)&pCB->secStatus);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_PRIMARY_BUS,
                         (UINT8 *)&pCB->priBus);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_SECONDARY_BUS,
        ...
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_SEC_LATENCY,
                         (UINT8 *)&pCB->secLatency);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_MEM_BASE_0,
                         (UINT32 *)&pCB->memBase0);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_MEM_LIMIT_0,
                         (UINT32 *)&pCB->memLimit0);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_MEM_BASE_1,
                         (UINT32 *)&pCB->memBase1);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_MEM_LIMIT_1,
                         (UINT32 *)&pCB->memLimit1);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_IO_BASE_0,
                         (UINT32 *)&pCB->ioBase0);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_IO_LIMIT_0,
                         (UINT32 *)&pCB->ioLimit0);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_IO_BASE_1,
                         (UINT32 *)&pCB->ioBase1);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_IO_LIMIT_1,
                         (UINT32 *)&pCB->ioLimit1);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BRG_INT_LINE,
                         (UINT8 *)&pCB->intLine);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BRG_INT_PIN,
        ...
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BRIDGE_CONTROL,
                         (UINT16 *)&pCB->control);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_SUB_VENDOR_ID,
                         (UINT16 *)&pCB->subVendorId);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_SUB_SYSTEM_ID,
                         (UINT16 *)&pCB->subSystemId);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_16BIT_LEGACY,
                         (UINT32 *)&pCB->legacyBase);
        vxbPciCBheaderPrint2 (pCB);

        if (pCB->status & PCI_STATUS_NEW_CAP)
            vxbPciCapabShow2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CB_CAP_PTR);
        ...
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BASE_ADDRESS_1,
                         (UINT32 *)&pD->base1);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BASE_ADDRESS_2,
                         (UINT32 *)&pD->base2);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BASE_ADDRESS_3,
                         (UINT32 *)&pD->base3);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BASE_ADDRESS_4,
                         (UINT32 *)&pD->base4);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_BASE_ADDRESS_5,
                         (UINT32 *)&pD->base5);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_CIS,
                         (UINT32 *)&pD->cis);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_SUB_VENDER_ID,
                         (UINT16 *)&pD->subVendorId);
        (void) vxbPciConfigInWord_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_SUB_SYSTEM_ID,
                         (UINT16 *)&pD->subSystemId);
        (void) vxbPciConfigInLong_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_EXPANSION_ROM,
                         (UINT32 *)&pD->romBase);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_DEV_INT_LINE,
                         (UINT8 *)&pD->intLine);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_DEV_INT_PIN,
                         (UINT8 *)&pD->intPin);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_MIN_GRANT,
                         (UINT8 *)&pD->minGrant);
        (void) vxbPciConfigInByte_2 (busCtrlID, busNo, deviceNo, funcNo, PCI_CFG_MAX_LATENCY,
                         (UINT8 *)&pD->maxLatency);
        vxbPciDheaderPrint2 (pD);
...
/*********************************************************************
*
* vxbPciFuncShow2 - show configuration details about a function
*
...
STATUS vxbPciFuncShow2
    ...
    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, PCI_CFG_VENDOR_ID,
                     (UINT16 *)&vendorId);

    (void) vxbPciConfigInWord_2 (busCtrlID, bus, device, function, PCI_CFG_DEVICE_ID,
                     (UINT16 *)&deviceId);

    (void) printf("[%d,%d,%d] - (%04x %04x) type=",bus,device,function, vendorId, deviceId);

    (void) vxbPciConfigInByte_2 (busCtrlID, bus, device, function,
    ...
    (void) vxbPciStatusWordShow2(busCtrlID,bus, device, function);
    (void) vxbPciCmdWordShow2(busCtrlID,bus, device, function);

    vxbPciFuncBarShow2(busCtrlID,bus, device, function, numBars);
...
/*********************************************************************
*
* vxbPciTopoShow2 - show PCI topology
*
    ...
    for (busNo = 0; busNo < PCI_MAX_BUS; busNo++)
    {
        ...
        for (deviceNo=0; deviceNo < 1; deviceNo++)  /* only for pcie_rc in PL*/
        {
            for (func = 0; func < 8; func++)
            {
                ...
                (void) vxbPciFuncShow2 (busCtrlID, busNo, deviceNo, func);
...
void vxbPciTopoShow1(void)    
{
	vxbPciTopoShow2(0);
	return;
}

STATUS vxbPciHeaderShow1(UINT8         busNo, UINT8 deviceNo, UINT8 funcNo)
{
	return vxbPciHeaderShow2(1, busNo, deviceNo, funcNo);
}
