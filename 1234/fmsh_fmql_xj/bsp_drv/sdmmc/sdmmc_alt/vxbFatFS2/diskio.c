...
)
{
	...
	if (pdrv == 1)
	{
		BaseAddress = FPAR_SDPS_1_BASEADDR;  /* sd_ctrl_1*/
	}
	else
	{
		BaseAddress = FPAR_SDPS_0_BASEADDR;  /* sd_ctrl_0*/
	}
...
)
{
	...
	#ifdef REF_ATL_EMMC_RN
		pDev = vxbInstByNameFind (ALT_DW_MSHC_NAME, unit);  /* ref_alt for ruineng 64G emmc*/
	#else
		pDev = vxbInstByNameFind (FM_SDMMC_NAME, unit);   /* for 7045 demo_board*/
	#endif
		if (pDev == NULL)
		{
			printf ("Can not find this sd unit(%d)!  \n", unit);
