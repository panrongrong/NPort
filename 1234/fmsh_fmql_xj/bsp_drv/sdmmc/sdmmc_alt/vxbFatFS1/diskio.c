...
)
{
	...
	#if 1		
		VXB_DEVICE_ID		pDev;
		FM_SDMMC_DRV_CTRL * pDrvCtrl;
		SDMMC_CARD *		card;
		
		int unit = pdrv;
		
		/*pDev = vxbInstByNameFind (FM_SDMMC_NAME, unit);*/
		pDev = vxbInstByNameFind ("altSocGen5DwMshc", unit);
		if (pDev == NULL)
		{
			printf ("Can not find this sd unit(%d)!  \n", unit);
			return RES_ERROR;
		}		
		pDrvCtrl= (FM_SDMMC_DRV_CTRL *) pDev->pDrvCtrl;
		card = &pDrvCtrl->card;
		
		/*Status = vxSdmmc_RdSector_Poll2(pSdmmc, (UINT8 *(&buff[0]), LocSector, len);*/
		/*Status = vxSdmmc_BlkRead_Poll (card, LocSector, count, (UINT8 *(&buff[0]));		*/
		Status = altSocGen5DwMshc_BlkRead_Poll (card, LocSector, count, (UINT8 *)(&buff[0]));		
		if (Status == ERROR) 
		{
			return RES_ERROR;
		}	
...
)
{
	...
	len = count * SDMMC_BLOCK_SIZE;	
		...
		/*Status = vxSdmmc_WrSector_Poll2(pSdmmc, (UINT8 *(&buff[0]), LocSector, len);*/
		/*Status = vxSdmmc_BlkWrite_Poll (card, LocSector, count, (UINT8 *(&buff[0]));		*/
		Status = altSocGen5DwMshc_BlkWrite_Poll (card, LocSector, count, (UINT8 *)(&buff[0]));		
		if (Status == ERROR) 
		{
			return RES_ERROR;
		}	
