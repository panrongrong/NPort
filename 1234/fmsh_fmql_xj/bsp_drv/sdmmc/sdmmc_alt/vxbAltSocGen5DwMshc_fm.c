...
#if 1
    pDrvCtrl->card.xbdDev.blkRd = (FUNCPTR)altSocGen5DwMshc_BlkRead_Poll;
    pDrvCtrl->card.xbdDev.blkWt = (FUNCPTR)altSocGen5DwMshc_BlkWrite_Poll;
#else
    ...
    )
{
	...
	if (((mintSts & ALT_MSHC_INT_CMD_DONE) != 0) )
        {
...
LOCAL void altSocGen5DwMshc_CardMonTask
    ...
    FOREVER
    {
        ...
        if (inserted && (pDrvCtrl->cardIns == FALSE))
        {
            ...
            rc = sdmmc_Identify_3 (&pDrvCtrl->card);  /* sd / mmc-big_capicity*/
		#endif
            if (rc == ERROR)
            {
            ...
            if (dev == NULLDEV)
            {
                ...
                printf ("sdMmcXbdXbdCreate() failed !!! \n");
        ...
        if (!inserted && (pDrvCtrl->cardIns == TRUE))
        {
            ...
            ALT_DW_MSHC_DBG (ALT_DW_MSHC_DBG_XBD, "Card removed\n",
                             1, 2, 3, 4, 5, 6);
#if 1  /* vxbus - dosfs		*/
            sdMmcXbdDevDelete (&pDrvCtrl->card);  /* for dosfs*/
#endif            
...
#else
	present = TRUE;  /* emmc-64g for ruineng*/
...
#if 0 /* for emmc card detect always ok*/
	...
	if (g_sdmmc_fs_mount == 0)
	{
		...
		*/
		if (res_sd == FR_NO_FILESYSTEM)
		{
			...
			res_sd = f_mkfs("0:", FM_FAT32, work, sizeof work); /* Create FAT volume on the logical drive 0 */
