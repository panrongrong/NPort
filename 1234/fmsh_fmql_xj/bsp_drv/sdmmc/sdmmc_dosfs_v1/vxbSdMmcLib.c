...
UINT32 sdMmcLibDbgMask = SDMMC_LIB_DBG_OFF; /* SDMMC_LIB_DBG_OFF */
...
LOCAL STATUS sdMmcHostCmdIssue
    ...
    if (rc == ERROR)
        {
        SDMMC_LIB_DBG (SDMMC_LIB_DBG_ERR, "CMD%d: issue error(sd_%d) \n",
                       cmd->cmdIdx, host->pDev->unitNumber, 0, 0, 0 ,0);
        }

    SDMMC_LIB_DBG (SDMMC_LIB_DBG_CMD, "CMD%d: issued(sd_%d) \n",
                   cmd->cmdIdx, host->pDev->unitNumber, 0, 0, 0 ,0);
...
STATUS sdMmcCmdDeselectCard
    ...
    SDMMC_CMD_FORM (cmd, SDMMC_CMD_SELECT_CARD, 0, SDMMC_CMD_RSP_R1B, FALSE);
...
STATUS sdMmcCmd_CMD1
	...
	UINT32 cmdArg = 0x40FF8000; 
	
	card->info.voltage = cmdArg;

    SDMMC_CMD_FORM (cmd, MMC_CMD_SEND_OP_COND, cmdArg, SDMMC_CMD_RSP_R3, FALSE);
    ...
    )
{
	...
	SDMMC_LIB_DBG (SDMMC_LIB_DBG_ALL, "\n\n ---sdMmcIdentify(%d):--- \n", \
		           card->host->pDev->unitNumber, 0,0,0,0,0);
	...
	*/
	rc = sdMmcCmdAppCmd (card);  /* cmd_55*/
	printf("\n **sdMmcCmdAppCmd: rc=%d ** \n", rc);	
	if (rc == ERROR)
	{	
		/* mmc / emmc */
		card->isMmc = 1;
		printf("==card:%d is emmc!== \n", card->host->pDev->unitNumber);
	}
	else
	{
		/* sd*/
...
*/
	...
	****       MMC/eMMC card        ****
	***********************************/
	if (card->isMmc == 1) /* jc : mmc/emmc*/
	{	
		/* #define MMC_CARD_BUSY	0x80000000	/* Card Power up status bit */
		while (!(card->info.voltage & 0x80000000)) /* MMC_CARD_BUSY */
		{
			/* ACMD41 to ask card to send its OCR */
			rc = sdMmcCmd_CMD1 (card);
			if (rc == OK)
				break;
