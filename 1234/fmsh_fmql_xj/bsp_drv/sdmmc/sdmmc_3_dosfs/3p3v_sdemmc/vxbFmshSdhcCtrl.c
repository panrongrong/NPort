    ...
    )
{
	...
	if (pSdCmd->hasData) 
	{
	   ...
	   if (pSdCmd->cmdData.blkNum > 1)  /* multi-block */
	   {
	...
	/*
	printf("prepare:blkNum=%d, pSdCmd->cmdid=%d cmdSend=0x%08X, cmdArg=0x%X \n", \
		    pSdCmd->cmdData.blkNum, pSdCmd->cmdIdx, cmdSend, pSdCmd->cmdArg);
	*/
    ...
    )
{
    ...
    while (pDrvCtrl->remainBytes != 0) 
	{
		...
		/*vxbUsDelay (100);*/
#ifdef SDMMC_NORMAL_SPEED    
		vxbUsDelay (10);  /* for high speed, disable*/
#endif
...
IMPORT void sysMshcClkFreqSetup_2(UINT32 clk, int ctrl);
void fmshSdhc_ClkFreqSetup(VXB_DEVICE_ID pDev, UINT32 clk)
{
	...
	sysMshcClkFreqSetup(clk);  /* test for bj_ruineng, old*/
	/*sysMshcClkFreqSetup_2(clk, ctrl_x);  // new_code*/
...
void test_sdmmc_speed_dosfs_2(char* file_name_in)
{
	float speed = 0.0;
	UINT32 all_size = 0;
	int tick_start = 0, tick_end = 0;
	...
	tick_start = tickGet();
	/*sprintf(file_name, "%s", "/sd0:0/zImage");*/
	sprintf(file_name, "%s", file_name_in);
	...
	taskDelay(10);	
	printf("read %d bytes data of file(%s) \r\n", all_size, file_name,0,0,0,0);
	...
	sprintf(file_name, "%s_%d.txt", file_name_in, file_idx_dosfs);
	file_idx_dosfs++;
	...
	*/
	...
	taskDelay(10);	
	printf("write %d bytes data of file(%s) \r\n", all_size, file_name, 0,0,0,0);
