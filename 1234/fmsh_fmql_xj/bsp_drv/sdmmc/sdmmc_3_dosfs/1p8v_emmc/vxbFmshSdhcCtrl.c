...
/**/
#undef  SDMMC_NORMAL_SPEED     /* high speed mode*/
/*#define  SDMMC_NORMAL_SPEED  // normal speed*/
...
LOCAL void fmshSdhcCtrl_Monitor
    ...
    {    
		...
		#ifdef SD_DRV_LIB_DEBUG_MODE  /* usr_define lib*/
...
LOCAL STATUS fmshSdhcCtrl_SendCmd
   ...
   /* printf("fmshSdhcCtrl_SendCmd:cmd=0x%x, arg=0x%x send  0x%x\n",cmd, arg, CSR_READ_4 (pDev, ALT_MSHC_CMD));*/
    ...
    for (i = 0; i < ALT_MSHC_MAX_RETRIES; i++) 
    {
        ...
        /**/
		/* bit9==0: sd/mmc not busy*/
		/**/
		if ((CSR_READ_4 (pDev, ALT_MSHC_STATUS) & (UINT32)ALT_MSHC_CMD_DATA_EXP) == 0)  
        {
            return OK;
        }
        vxbUsDelay(10);
    ...
    )
{
...
#else  /* ref_linux*/
	if ((cmdSend == SDMMC_CMD_STOP_TRANSMISSION) || \
	    (cmdSend ==  SDMMC_CMD_GO_IDLE_STATE) ||     \
	    (cmdSend == SDMMC_CMD_GO_INACTIVE_STATE) ||   \
	    ((cmdSend == 52/*SD_IO_RW_DIRECT*/) &&  (((pSdCmd->cmdArg >> 9) & 0x1FFFF) == 0x06/*SDIO_CCCR_ABORT*/)))
	{	
		cmdSend |= ALT_MSHC_CMD_STOP;
	}	
	else if ((pSdCmd->cmdIdx != SDMMC_CMD_SEND_STATUS) && pSdCmd->hasData)
	{	
		cmdSend |= ALT_MSHC_CMD_PRV_DAT_WAIT;
	}
#endif
	...
	if (pSdCmd->hasData) 
	   ...
	   if (!pSdCmd->cmdData.isRead)   /* write*/
	   {    
	...
	if (!pDrvCtrl->sdHostCtrl.polling)  /* irq_mode*/
	{
		...
		*pSdCmd = pDrvCtrl->pSdCmdLast;
		...
		else
		{
			...
			{
				SDHC_DBG (SDHC_DBG_ERR, 
				                "irq_fmshSdhcCtrl_CmdIssue: CMD%d error \n",
				                pSdCmd->cmdIdx, 2, 3, 4, 5, 6);
...
-rwxrwxrwx  1 0       0              8388608 Jan  1  1980 test6.txt 
-rwxrwxrwx  1 0       0              8388608 Jan  1  1980 test7.txt 
-rwxrwxrwx  1 0       0              8388608 Jan  1  1980 test8.txt 
value = 0 = 0x0
-> test_mmc_wr_file "test9.txt"
0x19fd0d8 (tShell0): write 8388608 bytes data of file(/mmc0:0/test9.txt) 
0x19fd0d8 (tShell0): data: 4_bit mode, speed:24999999 Hz 
test vx69_mmc_dosfs write_speed: 8987794.00 bytes/s 
value = 0 = 0x0
-> test_mmc_rd_file "test9.txt"
...
0x19fd0d8 (tShell0): read 8388608 bytes data of file(/mmc0:0/test9.txt) 
0x19fd0d8 (tShell0): data: 4_bit mode, speed:24999999 Hz 
test vx69_mmc_dosfs read_speed: 9320676.00 bytes/s 
value = 0 = 0x0
-> ll  "/mmc0:0"
...
-rwxrwxrwx  1 0       0              8388608 Jan  1  1980 test7.txt 
-rwxrwxrwx  1 0       0              8388608 Jan  1  1980 test8.txt 
-rwxrwxrwx  1 0       0              8388608 Jan  1  1980 test9.txt 
value = 0 = 0x0
-> 
*/

void test_mmc_wr_rd_file(int file_cnt)
{
	float speed = 0.0;
	UINT32 all_size = 0;
	int tick_start = 0, tick_end = 0;
	...
	for (cnt=0; cnt<file_cnt; cnt++)
	{
		...
		sprintf(file_name, "/mmc0:0/%s", "test_w_r.txt");
