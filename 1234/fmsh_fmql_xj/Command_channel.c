...
int  init_usart(UART_Config_Params *uart_instance,int client_socket, char *buf, int buf_len) 
{
	int ret;
	...
	//调用AXI_api设置串口相关寄存器
	axi165502CInit(uart_instance);

	uart_instance->config.usart_mcr_dtr = (int)buf[4];

	uart_instance->config.usart_mcr_rts = (int)buf[5];

	// 获取当前 MCR 寄存器值
	unsigned int mcr_reg = userAxiCfgRead(uart_instance->choose_channel, AXI_16550_MCR);
	...
	// 写入更新后的 MCR 寄存器值
	userAxiCfgWrite(uart_instance->choose_channel, AXI_16550_MCR, mcr_reg);

	uart_instance->config.usart_crtscts = (int)buf[6];


//	if(uart_instance->config.IX_on == (int)buf[7])
//	{
//		send_xon_xoff_char(uart_instance->choose_channel, 1);
//	}
//	if(uart_instance->config.IX_off == (int)buf[8])
//	{
//		send_xon_xoff_char(uart_instance->choose_channel, 0);
//	}
	...
	pack_buf[4] = 0x0; 
	//返回数据给中间件
	ret = socket_send_to_middle(uart_instance->sock_cmd, pack_buf, sizeof(pack_buf));
	if(ret < 0)
	{
		//printf("send error\n");
		return -1;
...
int  usart_set_baudrate(UART_Config_Params *uart_instance,int client_socket, char *buf, int buf_len)
{
	int ret;
	unsigned int baud_rate;
	...
	//调用AXI_api设置串口波特率
	//打包数据
	axi16550BaudInit(uart_instance->choose_channel, uart_instance->config.baud_rate);
	...
	response[2] = 'K';            // 'K' 的 ASCII 码：0x4B
	//返回数据给中间件
	ret = socket_send_to_middle(uart_instance->sock_cmd, response, sizeof(response));
	if(ret < 0)
	{
		//printf("send error\n");
		return -1;
...
void handle_command(UART_Config_Params *uart_instance,int client_socket, char *buf, int buf_len) 
{
	...
	case ASPP_CMD_PORT_INIT:
	{
		init_usart(uart_instance,client_socket, buf, data_len);
		break;
	}
	case ASPP_CMD_NOTIFY:
	{
	...
	case ASPP_CMD_XONXOFF:
	{
		usart_set_xon_xoff(uart_instance->sock_cmd,uart_instance->choose_channel, buf, data_len);
		break;
	}

	case ASPP_CMD_TX_FIFO:
	{
		usart_set_tx_fifo(uart_instance->sock_cmd,uart_instance->choose_channel, buf, data_len);
		break;
	}

	case ASPP_CMD_LINECTRL:
	{
		usart_set_line_control(uart_instance->sock_cmd,uart_instance->choose_channel, buf, data_len);
		break;
	}

	case ASPP_CMD_SETXON:
	{
		usart_set_xon(uart_instance->sock_cmd,uart_instance->choose_channel, buf, data_len);
		break;
	}

	case ASPP_CMD_SETXOFF:
	{
		usart_set_xoff(uart_instance->sock_cmd,uart_instance->choose_channel, buf, data_len);
		break;
	}

	case ASPP_CMD_START_BREAK:
	{
		usart_set_start_break(uart_instance->sock_cmd,uart_instance->choose_channel, buf, data_len);
		break;
	}

	case ASPP_CMD_STOP_BREAK:
	{   
		usart_set_stop_break(uart_instance->sock_cmd,uart_instance->choose_channel, buf, data_len);
...
//		usart_report_hearbeat(client_socket, buf, data_len);
	...
	case ASPP_CMD_WAIT_OQUEUE:
	{
		usart_report_queue(uart_instance->sock_cmd, buf, data_len);
		break;
	}
