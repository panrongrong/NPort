    ...
    )
{
    ...
    for (pNode = DLL_FIRST3((DL_LIST3 *)&pParent->children); pNode != NULL; pNode = DLL_NEXT3(pNode))
    {
        ...
        {
            vxbPciResCheck3 (busCtrlID, pPciDev, pSup64);
        }
    }
}
...
LOCAL void vxbPciResAdjust3
    (
    VXB_DEV_ID3     busCtrlID,
    PCI_ROOT_RES3 * pciRootRes,
    PCI_DEVICE3   * pPciDummy
    )
{
    PCI_DEVICE3* pPciDev;
    DL_NODE3 *   pNode = NULL;
    ...
    for (pNode = DLL_FIRST3((DL_LIST3 *)&pPciDummy->children); pNode != NULL; pNode = DLL_NEXT3(pNode))
    {
        pPciDev = (PCI_DEVICE3 * )pNode;

        if ((pciRootRes->barRes3[PCI_ROOT_RES_PREMEM64_IDX].size == 0) &&
            (pciRootRes->barRes3[PCI_ROOT_RES_PREMEM32_IDX].size == 0))
        {

            /* if no prefetch resource, just assign the none-prefetch resource */

            printf("bridge  (%d:%d:%d) doesn't support prefetch memory\n",
        ...
        vxbPciResCheck3 (busCtrlID, pPciDev, &sup64);

        if ((pciRootRes->barRes3[PCI_ROOT_RES_PREMEM64_IDX].size != 0) &&
            (sup64 == TRUE))
        {
            printf("bridge  (%d:%d:%d) use 64 bit prefetch memory \n",
                         pPciDev->bus, pPciDev->device, pPciDev->func);

            pPciDev->preIndex = PCI_ROOT_RES_PREMEM64_IDX;
            pPciDummy->preIndex = PCI_ROOT_RES_PREMEM64_IDX;
         }
         else if (pciRootRes->barRes3[PCI_ROOT_RES_PREMEM32_IDX].size != 0)
         {
            printf("bridge  (%d:%d:%d) use 32 bit prefetch memory \n",
                         pPciDev->bus, pPciDev->device, pPciDev->func);
         ...
         {
            printf("bridge  (%d:%d:%d) use none prefetch memory \n",
                         pPciDev->bus, pPciDev->device, pPciDev->func);
				...
				//pResPool = &pPciDev->resource[i];
				 if (pPciDev->parent == NULL)
                 {
                        ...
                        (i == PCI_BRIDGE_MEM_BASE_INDEX))
                    {
                        ...
                        pResPool = &pciRootRes->barRes3[i - PCI_BRIDGE_MEM_BASE_INDEX];
                    }
                    else
                    {
                        /* prefetch memory */
                        pResPool = &pciRootRes->barRes3[pPciDev->preIndex];
                    }
                }
                else
                {
...
STATUS vxbPciAutoConfigFunc3(void)
{
    ZYNQ7K_AXI2PCIE_DRV_CTRL3 *  pDrvCtrl;
	
    PCI_ROOT_CHAIN3 * pciRChain;
    PCI_ROOT_RES3   * pciRootRes;
	
    struct barRes3   bres[PCI_ROOT_RES_MAX_IDX];
...
#if 1
	// drvCtrl
    //pDrvCtrl = (ZYNQ7K_AXI2PCIE_DRV_CTRL3 *)malloc(sizeof (ZYNQ7K_AXI2PCIE_DRV_CTRL3));
    pDrvCtrl = (ZYNQ7K_AXI2PCIE_DRV_CTRL3 *)memAlloc_Usr(sizeof (ZYNQ7K_AXI2PCIE_DRV_CTRL3));

	// rootRes
    //pciRootRes = malloc(sizeof(PCI_ROOT_RES3));
    pciRootRes = memAlloc_Usr(sizeof(PCI_ROOT_RES3));

	// RChain
    pciRChain = &pDrvCtrl->rc;
	g_pciRChain = pciRChain;
	
    pciRChain->segCount = 1;
    pciRChain->autoConfig = TRUE;
	
    pciRootRes->baseBusNumber = 0;   // baseBus;
    
   // pciRootRes->endBusNumber = 4;  //endBus; -> maxBus_id  613_zhq
    pciRootRes->endBusNumber = 8;  //endBus; -> maxBus_id
	
    pciRChain->pRootRes = pciRootRes;
	
	/* ref_vx7:
	============
	bres[PCI_ROOT_RES_IO_IDX].base		= 0x6A000000 
	bres[PCI_ROOT_RES_IO_IDX].baseParent= 0x6A000000 
	bres[PCI_ROOT_RES_IO_IDX].len		= 0x100000 
	bres[PCI_ROOT_RES_IO_IDX].adjust	= 0x0 
	
	bres[PCI_ROOT_RES_MEM32_IDX].base	   = 0x60000000 
	bres[PCI_ROOT_RES_MEM32_IDX].baseParent= 0x60000000 
	bres[PCI_ROOT_RES_MEM32_IDX].len	   = 0xA000000 
	bres[PCI_ROOT_RES_MEM32_IDX].adjust    = 0x0 
	
	pDrvCtrl->memIo32Addr	 = 0x60000000 
	*/
  	/*
  	bres[PCI_ROOT_RES_IO_IDX].base       = (UINT64)XPAR_AXI_PCIE_0_AXIBAR_0;  //0x6A000000;     // iospace.basePci;
    bres[PCI_ROOT_RES_IO_IDX].baseParent = (UINT64)XPAR_AXI_PCIE_0_AXIBAR_0;  //0x6A000000;     // iospace.baseParent;
    bres[PCI_ROOT_RES_IO_IDX].len        = (UINT64)0x10000000;  // 0x100000;       // iospace.len;
    bres[PCI_ROOT_RES_IO_IDX].adjust     = (UINT64)0;              // iospace.baseParent - iospace.basePci;
	*/
	
    bres[PCI_ROOT_RES_MEM32_IDX].base       = (UINT64)XPAR_AXI_PCIE_0_AXIBAR_0;  //0x60000000;    // memspace.basePci;
    bres[PCI_ROOT_RES_MEM32_IDX].baseParent = (UINT64)XPAR_AXI_PCIE_0_AXIBAR_0;  // 0x60000000;    // memspace.baseParent;
    bres[PCI_ROOT_RES_MEM32_IDX].len        = (UINT64)(XPAR_AXI_PCIE_0_AXIBAR_HIGHADDR_0-XPAR_AXI_PCIE_0_AXIBAR_0 + 1);  // 0xA000000;    // memspace.len;
    bres[PCI_ROOT_RES_MEM32_IDX].adjust     = (UINT64)0x0;    // memspace.baseParent - memspace.basePci;

    pDrvCtrl->memIo32Addr =  XPAR_AXI_PCIE_0_AXIBAR_0;  //0x60000000;                    // memspace.basePci;

	
    vxbPciSetBarResPool3 (pciRootRes, bres);
	
#endif	

        
    for (i = 0; i < pciRChain->segCount; i++)
    {
        pciRootRes = (PCI_ROOT_RES3 *)&pciRChain->pRootRes[i];

        printf(10, "vxbPciAutoConfig(): bus number[%d - %d] pciRootRes %p\n",
		...
		初始化设备树顶点
		*/
	    //pPciDev = (PCI_DEVICE3 *)memAlloc_Usr();
	    //pPciDev = (PCI_DEVICE3 *)malloc(sizeof (PCI_DEVICE3));
	    pPciDev = (PCI_DEVICE3 *)memAlloc_Usr(sizeof (PCI_DEVICE3));
		...
		pPciDev->resource[PCI_BRIDGE_MEM_BASE_INDEX].pool.list.head = NULL;
		pPciDev->resource[PCI_BRIDGE_MEM_BASE_INDEX].pool.list.tail = NULL;
		DLL_INIT3((DL_LIST3 *)&pPciDev->children);
#endif
		...
		扫描所有有效设备，形成设备树链表
		*/
	    (void) vxbPciDeviceScan3 (busCtrlID, pciRootRes, pciRootRes->baseBusNumber, pPciDev);

	    if (!DLL_EMPTY3(&pPciDev->children))
	    {
			vxbPciResAdjust3 (busCtrlID, pciRootRes, pPciDev);
	    	...
	    	*/
	    	...
	    	根据资源树链表，分配实际的地址范围
	    	*/
	        vxbPciResourceAssign3 (busCtrlID, pciRootRes, pPciDev);
	...
	//vxbPciBusAddDev3 (busCtrlID, pciRootRes->baseBusNumber);
